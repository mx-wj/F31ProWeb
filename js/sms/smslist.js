...
function(_, $, ko, config, service, chosen) {
	...
	sendSmsClickHandler = function(){
		if ($("#chat-input").val() === "Please type message here" || 
		$("#chat-input").val() === "Ingrese la información aquí" || 
		$("#chat-input").val() === "Veuillez saisir un message dans ce champ" || 
		$("#chat-input").val() === "inserir o a informação aqui" || 
		$("#chat-input").val() ===  "Введите сообщение" ) {
			alert($.i18n.prop('sms_content_required'));
			return;
		}
		if(!hasCapability){
