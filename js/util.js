 ...
 */
 ...
 function checkBoxesSize(selectAll) {
     ...
     var arrlen = []
     theCheckbox.each(function(i, e) {
        if(e.getAttribute('checked')) {
            arrlen.push(i)
        }
    });

     var checkedBoxSize = arrlen.length;
 ...
 function checkSelectAll(selectAll, target) {
     var theCheckbox = $("#" + target + " .checkbox input:checkbox");
     if (selectAll[0]?.getAttribute('checked')) {
  ...
  * 翻译联网时长
  * 
  * @method transSecond2Time_day
  * @param {Number}
  *            secs 联网时间（秒）
  */
 function transSecond2Time_day(secs) {
     secs = parseInt(secs, 10);
     var isNegative = false;
     if(secs < 0){
         isNegative = true;
     ...
     var minu = Math.floor(secs / 60);
     secs = secs % 60;
     return (isNegative ? '-' : '') + leftInsert(days, 2, '0') + " Day : " +leftInsert(hour, 2, '0') + " Hour : " + leftInsert(minu, 2, '0') + " Min : " + leftInsert(secs, 2, '0')+" Sec";
 }
 ...
 */
 ...
 function rsrq_format(rsrq) {
     if (rsrq != "" || rsrq != 255) {
         return (parseInt(rsrq) - 40) / 2 + " dB";
     } else {
         return "--";
     }
 }
 
 function rscp_format(rscp) {
     if (rscp != "") {
           return (rscp-116) + " dBm";
     } else {
         return "--";
 ...
 function wfrcn_pci_fomart(wfrcn , pci) {
     if (wfrcn != "" & pci != "") {
           return wfrcn + "/" + pci;
     } else {
         return "-/-";
