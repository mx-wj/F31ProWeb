define(['jquery', 'knockout', 'config/config', 'service', 'underscore'],
    ...
    function ($, ko, config, service, _) {
        ...
        function IMEIVM() {
            var self = this;
            self.imei_enable = ko.observable();
            self.imei_value = ko.observable();
            self.imei = ko.observable();
            var data = service.getIMEIParams();
            self.imei_enable(data.imei_enable);
        ...
        if (localStorage.getItem('admin_root') == 1) {
            ...
            self.setIMEI = function () {
                showLoading('waiting');
                var params = {
                    imei_value: self.imei_value()
                };
                var res = service.setIMEI(params);
                if (res.result === 'success') {
                    //self.imei = (service.getIMEIParams().imei);
                    //$('#textCurrentIMEI').val(self.imei);
                    successOverlay();
		    init()
                    //location.reload();
                } else {
